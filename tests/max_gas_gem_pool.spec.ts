  ...
  for (let idx = 0; idx < INIT_STATES.length; idx++) {
    ...
    describe(`state #${idx}`, () => {
      ...
      describe('deploy', () => {
        ...
        it('upgrade_gem_pool (successed)', async () => {
          const {gem_pool_code} = await getGemFarmCodes()
          await wrapSend(st.deployer, st.gem_pool.sendMessage, toNano('50'), GemPool.msgUpgrade(gem_pool_code))(opt)
        })
      })
