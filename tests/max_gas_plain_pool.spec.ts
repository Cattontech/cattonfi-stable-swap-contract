  ...
  for (let idx = 0; idx < INIT_STATES.length; idx++) {
    ...
    describe(`state #${idx}`, () => {
      ...
      describe('deploy', () => {
        ...
        it('factory_admin_function (successed)', async () => {
          /** TranferAdmin */
          {
            // admin -> deployer
            await wrapSend(
              ...
              st.factory.sendMessage,
              toNano('50'),
              StableSwapFactory.msgTranferAdmin(st.deployer.address),
            )(opt, true)
            const {pools, ...factory_info} = await st.factory.getInfo()
            expect(factory_info.admin).toEqualAddress(st.deployer.address)
            expect(factory_info.fee_recipient).toEqualAddress(st.admin.address)
            expect(factory_info.is_permissionless).toBeTruthy()
            ...
            await wrapSend(
              ...
              st.factory.sendMessage,
              toNano('50'),
              StableSwapFactory.msgTranferAdmin(st.admin.address),
            )(opt, true)
          }
          ...
          /** NewFeeRecipient */
          {
            await wrapSend(
              ...
              st.factory.sendMessage,
              toNano('50'),
              StableSwapFactory.msgNewFeeRecipient(st.deployer.address),
            )(opt, true)
            const {pools, ...factory_info} = await st.factory.getInfo()
            expect(factory_info.fee_recipient).toEqualAddress(st.deployer.address)

            await wrapSend(
              st.admin,
              st.factory.sendMessage,
              toNano('50'),
              StableSwapFactory.msgNewFeeRecipient(st.admin.address),
            )(opt, true)
          }
          ...
          /** SetPermissionless */
          {
            await wrapSend(
              ...
              st.factory.sendMessage,
              toNano('50'),
              StableSwapFactory.msgSetPermissionless(false),
            )(opt, true)
            const {pools, ...factory_info} = await st.factory.getInfo()
            expect(factory_info.is_permissionless).toBeFalsy()

            await wrapSend(
              st.admin,
              st.factory.sendMessage,
              toNano('50'),
              StableSwapFactory.msgSetPermissionless(true),
            )(opt, true)
          }

          /** NewPlainPool */
          {
            const {pool_code, buffer_code, liquidity_code} = await getStableSwapCodes()
            await wrapSend(
              st.admin,
              st.factory.sendMessage,
              toNano('50'),
              StableSwapFactory.msgNewPlainPool({
          ...
          /** Upgrade */
          {
            const {factory_code} = await getStableSwapCodes()
            await wrapSend(
              st.admin,
              st.factory.sendMessage,
              toNano('50'),
              StableSwapFactory.msgUpgrade(factory_code),
            )(opt, true)
          }
        })
